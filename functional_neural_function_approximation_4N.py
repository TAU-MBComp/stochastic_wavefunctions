...
class GaussianLayer(tf.keras.layers.Layer):
    ...
    def call(self, input, bosonic):
        ...
        r3 = tf.slice(input, [0, tf.constant(2) * self.dim_physical], [-1, self.dim_physical])
        r4 = tf.slice(input, [0, tf.constant(3) * self.dim_physical], [-1, self.dim_physical])
        def psi(x, n):
            # return K.exp(-(x * x) / (2.0 * tf.math.square(self.sigma))) / tf.math.sqrt(2.0 * tf.constant(pi)) / self.sigma
            return K.exp(-(x * x) / (tf.constant(2.0) * self.sigma))
        def wavefunc_0(x):
            return tf.reduce_prod(psi(x,0), axis=1, keepdims=True)
        return wavefunc_0(r1)*wavefunc_0(r2)*wavefunc_0(r3)*wavefunc_0(r4)
...
def neural_fit(x, psi, n_samples, perm_sub, perm, parity, analysis_data, iteration, load_weights, bosonic, U, n_particles, dim_physical, n_layers, layer_size, epochs, batch_size, reg, normalize=True):
    ...
    def create_model():
        ...
        boundary = GaussianLayer(n_particles, dim_physical, sigma_init=3.0)(input, bosonic)
        ...
        opt = keras.optimizers.SGD(lr=0.5, decay=1e-4, momentum=0.9, nesterov=False)
        # opt = keras.optimizers.SGD(lr=0.1, decay=1e-5, momentum=0.9, nesterov=False)
...
if __name__ == '__main__':
    ...
    samples, psi = permute(samples, psi, perm, parity, dim_physical, n_particles)
