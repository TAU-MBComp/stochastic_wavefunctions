...
class GaussianLayer(tf.keras.layers.Layer):
    ...
    def call(self, input, bosonic):
        ...
        r5 = tf.slice(input, [0, tf.constant(4) * self.dim_physical], [-1, self.dim_physical])
        def psi(x, n):
        ...
        return wavefunc_0(r1)*wavefunc_0(r2)*wavefunc_0(r3)*wavefunc_0(r4)*wavefunc_0(r5)
...
def neural_fit(x, psi, n_samples, perm_sub, perm, parity, analysis_data, iteration, load_weights, bosonic, U, n_particles, dim_physical, n_layers, layer_size, epochs, batch_size, reg, normalize=True):
    ...
    def create_model():
        ...
        opt = keras.optimizers.SGD(lr=0.5, decay=1e-5, momentum=0.9, nesterov=False)
        # opt = keras.optimizers.SGD(lr=0.05, decay=1e-5, momentum=0.9, nesterov=False)
