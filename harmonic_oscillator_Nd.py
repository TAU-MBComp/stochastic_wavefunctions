...
def eigenfunction_samples(n, x, n_particles, dim_physical, offset, hbar, m, omega, bosonic):
        ...
        for n_p in range(n_particles):
            ...
            for d in range(dim_physical):
                if (bosonic):
                    ...
                    if d == (dim_physical-1):
                        row[d, n_p] = psi_single_particle(x_p[:,d], n+p[n_p])
                    else:
                        row[d, n_p] = psi_single_particle(x_p[:,d], n)
        if bosonic == 1:
...
def propagate_in_time(psi0, X, t, U, nu, hbar, m, omega, method="exact", normalize=False):
    # Setup
    ...
    if method == "exact":
        ...
        for i in range(1, n_t):
            ...
            average_value = np.sqrt(np.sum(np.abs(psi_t[:, i])**2 * np.abs(dx)))
            if normalize:
                psi_t[:, i] = psi_t[:, i] / average_value
    if method == "euler":
        H = hamiltonian(X, U, nu, hbar, m, omega)
        for i in range(1, n_t):
            psi_t[:, i] = psi_t[:, i - 1] - (1j * dt / hbar) * (H @ psi_t[:, i - 1])
            average_value = np.sqrt(np.sum(np.abs(psi_t[:, i])**2 * np.abs(dx)))
            if normalize:
                psi_t[:, i] = psi_t[:, i] / average_value
...
if __name__ == '__main__':
    #from plotting import show_wavefunction_animation_2d
    import matplotlib
    #matplotlib.use('Agg')
    ...
    def wavefunction(x):
        return eigenfunction_samples(0, x, n_particles, dim_physical, offset, hbar, m, omega, bosonic)
    def P0(x):
    ...
    #samples = sample_uniform(a, [int(1.0 * nsamples), *x0.shape])
    #samples = vec_sample_from_distribution(P0, x0, nsamples, decorrelation_steps, xmax)

    samples = np.zeros((nsamples, n_particles*dim_physical), dtype=float)
    X = sample_mixed(P0, x0, step, a, nsamples, decorrelation_steps, uniform_ratio)
    print('samples_shape = ', samples.shape)
    samples[:,0] = X[:,0] 
    samples[:,2] = X[:,2]    

    psi0 = eigenfunction_samples(0, samples.T, n_particles, dim_physical, offset, hbar, m, omega, bosonic=bosonic)
    #psi0_grid = eigenfunction_samples(0, X, n_particles, offset, hbar, m, omega, bosonic=bosonic)
    
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(samples[:,0], samples[:,2], np.real(psi0))
    #fig.savefig('HO_Nd_sampled.pdf')
