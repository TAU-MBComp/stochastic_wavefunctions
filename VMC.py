...
def full_potential(x):
    return potential(x) + tf.math.multiply_no_nan(body_potential(x), interactions)
...
class boundary(keras.layers.Layer):
    def __init__(self, name=None, **kwargs):
        ...
        self.alpha = tf.Variable(initial_value=2. / (x_max - x_min), trainable=False, name="a")
        ...
        self.listup1 = tf.Variable(initial_value=tf.random.uniform([spin_up, 1], minval=-1, maxval=1),
                                   trainable=True, name="detup1",
                                   shape=[spin_up, 1])
        self.listup2 = tf.Variable(initial_value=tf.random.uniform([1]), trainable=True, name="detup2",
                                   shape=[1])
        self.listdown1 = tf.Variable(initial_value=tf.random.uniform([spin_down, 1], minval=-1, maxval=1),
                                     trainable=True, name="detdown1",
                                     shape=[spin_down, 1])
        self.listdown2 = tf.Variable(initial_value=tf.random.uniform([1]), trainable=True, name="detdown2",
                                     shape=[1])
        # if symmetry == -1:
    ...
    if m == "box2d":
        def call(self, x):
            ...
            return bound_func(x1) * bound_func(
                x2)  # * tf.sqrt((x1 - 3. * tf.sin(np.pi *x1 / 5.) - x2)**2. + 0.05)#tf.sqrt((bound_func(x1) * bound_func(x2) * (x1 + 0.7*tf.sin(2.*x1*np.pi) / np.pi -x2))**2 + epsilon**2.)
    ...
    if d // p == 3:
        ...
        def call(self, x):
            ...
            if spin_up > 0:
                x_up = x[:, 0 * (d // p):(d // p) * spin_up]
                symdetup = detn(symmetry_detplus(x_up, spin_up, list_up, a_up), spin_up)
            if spin_down > 0:
                x_down = x[:, spin_up * (d // p):(d // p) * p]
                symdetdown = detn(symmetry_detplus(x_down, spin_down, list_down, a_down), spin_down)
...
def run():
    ...
    def d2(x):
        ...
        return tf.reduce_sum(tf.linalg.diag_part(laplace), axis=1, keepdims=True)
